...
(0, server_ts_1.serve)(async (req) => {
    ...
    try {
        ...
        const { booking_id, max_radius_km = 10 } = await req.json();
        if (!booking_id) {
        ...
        if (booking.status !== 'pending') {
            return new Response(JSON.stringify({ error: 'Booking is not in pending status' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        if (booking.driver_id) {
            return new Response(JSON.stringify({ error: 'Booking already has a driver assigned' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        `)
            ...
            if (allDriversError || !allDrivers) {
                return new Response(JSON.stringify({ error: 'No drivers available', assigned: false }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
            }
            ...
            availableDrivers = allDrivers
                ...
                .map((d) => ({
                ...d,
                distance_km: calculateDistance(booking.origin_latitude, booking.origin_longitude, d.current_latitude, d.current_longitude),
            }))
        ...
        if (updateError) {
            return new Response(JSON.stringify({ error: 'Failed to assign driver', details: updateError.message }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        return new Response(JSON.stringify({
            ...
            driver: {
                ...
                name: (_a = assignedDriver.user) === null || _a === void 0 ? void 0 : _a.name,
                phone: (_b = assignedDriver.user) === null || _b === void 0 ? void 0 : _b.phone,
                avatar_url: (_c = assignedDriver.user) === null || _c === void 0 ? void 0 : _c.avatar_url,
