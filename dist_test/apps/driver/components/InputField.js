...
const InputField = ({ label, icon, secureTextEntry = false, labelStyle, containerStyle, inputStyle, iconStyle, className, ...props }) => {
    return (<react_native_1.View className={`my-2 w-full ${containerStyle || ''}`}>
      <react_native_1.Text className={`text-lg font-JakartaSemiBold mb-3 ${labelStyle || ''}`}>
        {label}
      </react_native_1.Text>
      <react_native_1.View className="flex flex-row justify-start items-center relative bg-neutral-100 rounded-full border border-neutral-100 focus:border-primary-500">
