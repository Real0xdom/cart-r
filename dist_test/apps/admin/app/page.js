...
function Home() {
    ...
    (0, react_1.useEffect)(() => {
        async function fetchData() {
            try {
                ...
                const [usersRes, driversRes, bookingsRes, onlineDriversRes, pendingRes, completedRes] = await Promise.all([
                    ...
                    supabase_1.supabase.from('drivers').select('*', { count: 'exact', head: true }),
                    supabase_1.supabase.from('bookings').select('*', { count: 'exact', head: true }),
                    supabase_1.supabase.from('drivers').select('*', { count: 'exact', head: true }).eq('is_online', true),
                    supabase_1.supabase.from('bookings').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
                    supabase_1.supabase.from('bookings').select('total_fare').eq('status', 'completed'),
                ]);
        ...
        const subscription = supabase_1.supabase
            ...
            setRecentBookings(prev => [payload.new, ...prev.slice(0, 9)]);
            setStats(prev => ({
                ...prev,
      ...
      <div className="ml-72 p-8 max-w-[1600px]">
        <div className="flex justify-between items-center mb-10">
          ...
          <div className="flex items-center gap-4">
            ...
            {stats.pendingBookings > 0 && (<span className="px-4 py-2 bg-orange-100 text-orange-700 rounded-xl text-sm font-semibold animate-pulse shadow-sm border border-orange-200">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow group">
            <div className="flex justify-between items-start mb-4">
              <div className="p-3 bg-emerald-50 rounded-xl group-hover:bg-emerald-100 transition-colors">
                <lucide_react_1.Users size={22} className="text-emerald-600"/>
              </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow group">
            <div className="flex justify-between items-start mb-4">
              <div className="p-3 bg-orange-50 rounded-xl group-hover:bg-orange-100 transition-colors">
                <lucide_react_1.Truck size={22} className="text-orange-600"/>
              </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow group">
            <div className="flex justify-between items-start mb-4">
              <div className="p-3 bg-green-50 rounded-xl group-hover:bg-green-100 transition-colors">
                <lucide_react_1.Package size={22} className="text-green-600"/>
              </div>
              {stats.pendingBookings > 0 && (<span className="text-xs font-semibold text-orange-600 bg-orange-50 px-2 py-1 rounded-lg">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow group">
            <div className="flex justify-between items-start mb-4">
              <div className="p-3 bg-purple-50 rounded-xl group-hover:bg-purple-100 transition-colors">
                <lucide_react_1.IndianRupee size={22} className="text-purple-600"/>
              </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-white">
            ...
            <link_1.default href="/bookings" className="px-5 py-2.5 bg-gray-900 text-white rounded-xl text-sm font-medium hover:bg-gray-800 transition shadow-lg shadow-gray-200">
              View All Bookings
            </link_1.default>
          </div>
          ...
          {loading ? (<div className="p-12 text-center text-gray-500">
              ...
              Loading data...
            </div>) : recentBookings.length === 0 ? (<div className="p-12 text-center text-gray-500 bg-gray-50/50">No bookings yet</div>) : (<div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-100">
                  {recentBookings.map((booking) => (<tr key={booking.id} className="hover:bg-gray-50/80 transition-colors group">
                      <td className="px-8 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-orange-50 flex items-center justify-center">
                            {booking.vehicle_type === 'bike' ? <lucide_react_1.Bike size={20} className="text-orange-600"/> : <lucide_react_1.Car size={20} className="text-orange-600"/>}
                          </div>
                          <div>
