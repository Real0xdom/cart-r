    ...
    return (<react_native_1.ScrollView className="flex-1 bg-white">
            <react_native_1.View className="flex-1 bg-white">
                <react_native_1.View className="relative w-full h-[250px]">
                    <react_native_1.Image source={constants_1.images.onboarding2} className="z-0 w-full h-[250px]"/>
                ...
                <react_native_1.View className="p-5">
                    {step === 'phone' ? (<>
                            <react_native_1.View className="mb-6">
                                <react_native_1.Text className="text-gray-500 mb-2">Sign in to book your next ride.</react_native_1.Text>
                            </react_native_1.View>

                            <InputField_1.default label="Mobile Number" placeholder="Enter phone with country code" icon={constants_1.icons.email} value={form.phone} onChangeText={(value) => setForm({ ...form, phone: value })} keyboardType="phone-pad"/>
                            <CustomButton_1.default title={loading ? "Sending OTP..." : "Continue"} onPress={onSignInPress} className="mt-6 bg-primary-500" disabled={loading}/>
                        </>) : (<>
                            <InputField_1.default label="One Time Password" placeholder="Enter 6-digit code" icon={constants_1.icons.lock} value={form.otp} onChangeText={(value) => setForm({ ...form, otp: value })} keyboardType="number-pad"/>
                            <react_native_1.View className="mt-6 gap-2">
                                <CustomButton_1.default title={loading ? "Verifying..." : "Verify & Login"} onPress={onVerifyPress} className="bg-primary-500" disabled={loading}/>
                                <CustomButton_1.default title="Back" onPress={() => setStep('phone')} className="bg-gray-100" textVariant="secondary" disabled={loading}/>
                            </react_native_1.View>
                        </>)}

                    <react_native_1.TouchableOpacity onPress={() => expo_router_1.router.replace('/')} className="mt-10 items-center">
                        <react_native_1.Text className="text-gray-400">Back to selection</react_native_1.Text>
