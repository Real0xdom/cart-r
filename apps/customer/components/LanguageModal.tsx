  ...
  return (
    ...
    >
      <Pressable className="flex-1 justify-end bg-black/50" onPress={onClose}>
        <Pressable
          className="bg-white rounded-t-3xl pt-6 pb-10 px-5"
          onPress={(e) => e.stopPropagation()}
        >
          <Text className="text-lg font-JakartaSemiBold text-gray-900 mb-1">
            {t("selectLanguage")}
          </Text>
          <Text className="text-sm text-gray-500 mb-4">{t("currentLanguage")}: {language === "en" ? t("english") : t("hindi")}</Text>

          {LANGUAGES.map(({ locale, labelEn, labelHi }) => (
            <TouchableOpacity
              key={locale}
              onPress={() => handleSelect(locale)}
              className={`py-4 px-4 rounded-xl mb-2 ${language === locale ? "bg-green-50 border border-green-200" : "bg-gray-50"}`}
              activeOpacity={0.7}
            >
              <Text className={`font-JakartaMedium ${language === locale ? "text-green-700" : "text-gray-800"}`}>
