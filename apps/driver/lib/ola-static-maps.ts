...
export async function generateStaticMap(options: StaticMapOptions): Promise<string> {
  ...
  // Note: URL.createObjectURL is not available globally in React Native
  // But it might be polyfilled or used with a library. 
  // However, for static maps, we usually just return the URL string.
  return url;
}
