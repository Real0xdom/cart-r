...
async function runHappyPath() {
    ...
    try {
        ...
        console.log(`[HappyPath] Users ready. Cust: ${customer.id.slice(0,5)}, Driver: ${driverId?.slice(0,5)}`);
        ...
        if (acceptError) throw new Error(`Driver Accept Failed: ${acceptError.message}`);
        ...
        if (b1.status !== 'accepted') throw new Error('Status mismatch after accept');
        ...
        if (startError) throw new Error(`Start Trip Failed: ${startError.message}`);
        ...
        if (completeError) throw new Error(`Complete Trip Failed: ${completeError.message}`);
