...
async function startRideRequestProgressUpdates(
  ...
  const updateNotification = async () => {
    ...
    // Always use the freshest data available for this booking (e.g. from an UPDATE event that added addons)
    const currentData = latestRideRequestData.get(bookingId) || data;

    await notifee.displayNotification(
      buildRideRequestNotification(
        { ...currentData, available_at: availableAt, expires_at_ms: rpcTimeoutAt, local_cancel_at_ms: localCancelAt },
        {
          channelId,
          remainingSeconds,
          availableAt,
...
TaskManager.defineTask(BACKGROUND_NOTIFICATION_TASK, async ({ data, error }) => {
  ...
  if (payload?.is_data_only || payload?.type === 'new_booking') {
    try {
      ...
      // Fetch full booking details in background to ensure addons and joined data are present
      // real-time push payloads usually only contain the raw database row.
      const bookingId = getRideRequestBookingId(parsedPayload);
      const { data: fullBooking } = await getBookingById(bookingId);
      
      await displayRideRequestWithStackLogic(fullBooking || parsedPayload);
    } catch (taskError) {
